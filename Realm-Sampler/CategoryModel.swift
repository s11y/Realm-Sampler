...
class CategoryModel: Object {
    ...
    static func lastId() -> Int {
        // idの最大値を取得してから、+1して返す
        if let category = realm.allObjects(ofType: CategoryModel.self).sorted(onProperty: "id", ascending: false).first {
    ...
    static func loadAll() -> [CategoryModel] {
        // idでソートして全件取得
        let categories = realm.allObjects(ofType: CategoryModel.self).sorted(onProperty: "id", ascending: true)
        // 取得したデータを配列に入れる
